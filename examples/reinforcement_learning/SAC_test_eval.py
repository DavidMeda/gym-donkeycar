...
if __name__ == "__main__":
    ...
    # env = DonkeyViewWrapper(env, os.path.join(log_dir, "encoder_1000_transfer_best.pkl"))
    # env = DonkeyViewWrapper(env, os.path.join(log_dir, "encoder_1000.pkl"))
    ...
    log.writerow(['Episode', 'Timestep', 'Avg Steer', 'Min Reward',
                  ...
                  "Average Absolute CTE", "Min Absolute CTE", "Max Absolute CTE", "Hit", "Num lap", 
                  "Avg time lap", "Best time lap", "Exit right lane", "Percent exit right lane"])
    try:
        ...
        for episode in range(n_episode):
            ...
            while not done:
                ...
                #     done = True
                if time_step >= 2700:
                    print(f" Stopping EVALUATION with TOT {time_step} steps, the SAC model reached laps={num_lap}")
                    done = True
                ...
                if done:
                    ...
                    print("FINISH EPISODE:", episode, f"(timestep: {time_step})" , " sum reward:",
                          ...
                          "avg time laps: ", round(avg_time_lap, 6), "Best time laps: ", round(best_time_lap, 6),
                    ...
                    log.writerow([episode, time_step_tot, round(np.mean(steers), 4), round(np.min(rewards), 4),
                        ...
                        round(avg_time_lap, 6), round(best_time_lap, 6),
                        exit_right_lane, perc_exit])
    ...
    dt = pd.read_csv(os.path.join(str(log_dir), name_model + "_test_metric.csv"))
    print(
        "\nFINAL RESULT\n",
        "\nAvg Steer ", round(dt["Avg Steer"].mean(), 4),
        "\nMin Reward ", round(dt["Min Reward"].min(), 4),
        "\nAvg Reward", round(dt["Avg Reward"].mean(), 4),
        "\nMax Reward", round(dt["Max Reward"].max(), 4),
        "\nReward Sum", round(dt["Reward Sum"].mean(), 4),
        "\nAvg Speed", round(dt["Avg Speed"].mean(), 4),
        "\nMax Speed", round(dt["Max Speed"].max(), 4),
        "\nMin CTE", round(dt["Min CTE"].min(), 4),
        "\nAvg CTE", round(dt["Avg CTE"].mean(), 4),
        "\nMax CTE", round(dt["Max CTE"].max(), 4),
        "\nAvg Distance", round(dt["Distance"].mean(), 4),
        "\nSum Distance", round(dt["Distance"].sum(), 4),
        "\nMin Throttle", round(dt["Min Throttle"].min(), 4),
        "\nAverage Throttle", round(dt["Average Throttle"].mean(), 4),
        "\nMax Throttle", round(dt["Max Throttle"].max(), 4),
        "\nAvg time lap", round(dt["Avg time lap"].mean(), 4),
        "\nBest time lap", round(dt["Best time lap"].min(), 4),
        "\nAvg Exit error", round(dt["Percent exit right lane"].mean(), 4), " %",
        "\n"
    )
    print("DONE TEST")
